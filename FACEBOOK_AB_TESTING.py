...
### EĞER VARYANS HOMOJENLIĞI VARSAYIMI SAĞLANMASAYDI İKİ GRUP FARKLILIĞINI KARŞILAŞTIRMAK İÇİN WILTCH TESTİ KULLANILIRDI.
...
## SONUÇLARA ULAŞMAK İÇİN TESTLER TEKRARLANMALIDIR. BÖYLE BİR İMKAN YA DA VAKİT YOKSA MEVCUR KULLANILAN ÜRÜN İLE
...
control_prop = df_control["Purchase"].sum()/df_control["Impression"].sum() * 100   ### %5
test_prop = df_test["Purchase"].sum()/df_test["Impression"].sum() * 100            ### %4       ## ORANDA DÜŞÜŞ VAR
...
control_prop = df_control["Click"].sum()/df_control["Impression"].sum() * 100      ### %5
test_prop = df_test["Click"].sum()/df_test["Impression"].sum() * 100               ### %3   ## ORANDA DÜŞÜŞ VAR
